...
class MyNeuralNetwork(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=3, padding=1, padding_mode="replicate")
        # Conv2D: 32 Filters. 
...
# I had to reduce the number of filter find on internet but also to reduce the number of fc neurons in the last layers.
# Indeed, I don't have a NVIDIA GPU but a Intel and I don't know how to reach it
...
def plot(train_history, test_history, metric, num_epochs, model_name):
    ...
    plt.legend()
    save_path = os.path.join(RESULT_DIR, f"{model_name}_{metric}.png")
...
print(f"Model to test :{[model.name() for model in Model]}")
for model in Model:
    ...
    with torch.no_grad():
        ...
        for i in range(6):
            ...
            plt.xticks([])
            plt.yticks([])
        save_path_ex = os.path.join(RESULT_DIR, f"{model.name()}_examples.png")
